...
```{r ds_plot, eval=FALSE, echo=TRUE, warning=FALSE, message=FALSE}
library(sf)
library(dplyr)
library(tmap)
...
where \(\beta_0\) is the intercept, \(\beta_{st}\) are coefficients vector of the random effects \(X_{st}\), which can be spatial, temporal or spatio-temporal (we have in our dataset population density (spatial only variable), inflation index (temporal only variable), temperature and rainfall (spatio-temporal variables)). In addition we can include fixed effects to the model \(s_i,\gamma_t \,\delta_{it}\), which are spatial effects, temporal effects and spatio-temporal effects respectively. 

We are using R INLA package (reference) to fit the models. It uses integrated nested laplacian approximation to estimate the parameters (fixed and random effects) and hyper-parameters of the model. To estimate random effects, INLA consider these to follow gaussian random markow field (GRMF), which indicate that observation is only related to its neighbors (two observations are independent if they are not neighbors).
...
## Spatio-temporal Interaction Term

The spatio-temporal interaction term $\delta_{i,t'}$ is modeled using a Type I interaction in the classification of (Give reference- Knorr-Held (2000). This specification combines a spatial Besag proper conditional autoregressive (CAR) model with an independent temporal structure. It allows each temporal block to have its own spatial deviation surface, capturing short-lived, localized crime anomalies that are not explained by the main spatial or temporal components.

Let $n$ denote the number of spatial areas and $T$ the number of temporal 
blocks (e.g., 2-week or 4-week periods). 
We stack the interaction terms into the vector
\[
\delta 
= 
(\delta_{1,1},\delta_{2,1},\ldots,\delta_{n,1},
 \delta_{1,2},\ldots,\delta_{n,2},
 \ldots,
 \delta_{n,T})^\top 
\sim 
\mathcal{N}(0, Q_{\mathrm{ST}}^{-1} ) .
\]

\paragraph{Kronecker-product precision structure.}
The joint precision matrix has a separable structure,
\[
Q_{\mathrm{ST}}
=
Q_{\mathrm{IID}}
...
Here,
\[
Q_{\mathrm{IID}} = \tau_t I_T
\]
is the temporal precision matrix representing independent shocks across 
time blocks, and $Q_{\mathrm{CAR}}$ is the Besag proper spatial precision matrix.

\paragraph{Spatial Besag proper component.}
For each time block $t'$, the spatial field satisfies a proper CAR model 
with precision
\[
Q_{\mathrm{CAR}}(i,i) = \tau_\delta (n_i + d),
\qquad
Q_{\mathrm{CAR}}(i,j) = -\tau_\delta 
\quad \text{if } i \sim j,
\]
and zero otherwise.
Here $n_i$ is the number of neighbours of area $i$, 
$\tau_\delta$ is the spatial precision hyperparameter, 
and $d>0$ is the diagonal inflation that ensures propriety of the CAR model.

\paragraph{Temporal IID component.}
Across temporal blocks, the spatial fields are assumed independent,
\[
\delta^{(1)}, 
\delta^{(2)}, 
\ldots,
...
\quad \text{are IID},
\]
with temporal precision matrix $\tau_t I_T$. 
This structure introduces no temporal smoothing, allowing spatial deviations 
to appear and disappear freely from one period to the next.

\paragraph{Final block-diagonal precision.}
The full precision matrix is therefore block diagonal,
\[
Q_{\mathrm{ST}}
=
\begin{pmatrix}
Q_{\mathrm{CAR}} & 0 & \cdots & 0 \\
0 & Q_{\mathrm{CAR}} & \cdots & 0 \\
\vdots & \vdots & \ddots & \vdots \\
0 & 0 & \cdots & Q_{\mathrm{CAR}}
\end{pmatrix},
\]
with one spatial CAR field for each temporal block.
This formulation captures spatially structured short-term crime anomalies 
without imposing temporal autocorrelation across blocks.
